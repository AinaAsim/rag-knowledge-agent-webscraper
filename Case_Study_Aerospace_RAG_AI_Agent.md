...
Crawls corporate websites, splits text into 1,000-character chunks, converts text to vectors, and upserts them to Pinecone.

### 2. Query Retriever (`02_query_retriever.json`)
Converts user queries to vectors, queries the vector database, and passes the context to GPT-4o to generate grounded responses.
...
🚀 **Case Study: Building a Zero-Hallucination RAG AI Support Agent for an Aerospace Launch Provider in Germany**

Automating customer support in safety-critical industries like aerospace leaves zero room for error. Hallucinations can result in compliance violations or severe reputational damage.

I designed and engineered **Orion**, a Retrieval-Augmented Generation (RAG) AI Support Agent integrated into the corporate website of a German rocket launch manufacturer:

* **50+ Pages Indexed**: Automates sitemap crawling, text chunking, and embedding.
* **Strict Context Guardrails**: Prompt engineering restricts responses to the verified database.
* **Markdown Table Formatting**: Converts HTML tables to Markdown, ensuring accurate technical data retrieval.
* **Low-Latency Search**: Queries vector databases and returns context in under 1.5 seconds.

**Results**:
* Automates pre-sales technical support questions.
* Prevents hallucinations by grounding AI responses in verified documentation.
* Updates sitemaps automatically, indexing new pages as the company website grows.

Read the full case study: [Case Study Link]

#AI #Aerospace #RAG #n8n #OpenAI #VectorDatabase #Pinecone #TechnicalWriting
...
### Problem
F&B and aerospace brands need accurate customer support, but manual processes are slow, and standard LLMs hallucinate technical specifications.

### Solution
Built a Retrieval-Augmented Generation (RAG) AI Support Agent using n8n, OpenAI Embeddings, and Pinecone to crawl, index, and query 50+ pages of launch documentation.

### Results
* Technical questions answered using verified company documentation.
* Hallucinations prevented through strict prompt constraints.
* Ingestion pipeline updates automatically, indexing new pages as the website grows.

### Tech Stack
n8n, OpenAI GPT-4o, Pinecone, Cheerio.
